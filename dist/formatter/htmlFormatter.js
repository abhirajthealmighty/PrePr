...
import { mergeRecommendation } from "../scoring/calculateScore.js";
...
function mergeRisk(violations) {
    const high = violations.filter(v => v.severity === "high").length;
    const medium = violations.filter(v => v.severity === "medium").length;
    if (high > 0)
        return { label: "HIGH", color: "#ef4444", icon: "🚫" };
    if (medium > 2)
        return { label: "MEDIUM", color: "#f59e0b", icon: "⚠️" };
    if (medium > 0)
        return { label: "LOW", color: "#f59e0b", icon: "🟡" };
...
function renderViolations(violations, metrics) {
...
function renderFixPriority(fixPriority) {
    if (!fixPriority.length)
        return "";
    let items = "";
...
export function generateHtmlReport(violations, metrics, score, breakdown = [], fixPriority = []) {
    ...
    const reviewMins = Math.round((metrics.filesChanged * 3) +
